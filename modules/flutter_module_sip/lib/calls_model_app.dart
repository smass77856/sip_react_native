...
class AppCallsModel extends CallsModel {
  ...
  /// Callback for showing local notification on incoming call (macOS)
  Function(String callerName, String callerNumber)?
  onShowIncomingCallNotification;
  ...
  void onIncomingSip(
    ...
    if (Platform.isIOS) {
      ...
      // Show local notification for incoming call on macOS
      _logs?.print('onIncomingSip macOS callId:$callId from:$hdrFrom');

      // Extract caller info from SIP headers
      String callerName = _extractDisplayName(hdrFrom);
      String callerNumber = _extractNumber(hdrFrom);

      // Trigger notification callback
      onShowIncomingCallNotification?.call(callerName, callerNumber);
    }
  }

  /// Extract display name from SIP header (e.g., "John Doe" <sip:123@domain.com>)
  String _extractDisplayName(String header) {
    final match = RegExp(r'"([^"]+)"').firstMatch(header);
    if (match != null) return match.group(1) ?? 'Unknown';

    final match2 = RegExp(r'([^<]+)<').firstMatch(header);
    if (match2 != null) return match2.group(1)?.trim() ?? 'Unknown';

    return 'Unknown';
  }

  /// Extract number from SIP header (e.g., sip:123@domain.com -> 123)
  String _extractNumber(String header) {
    final match = RegExp(r'sip:([^@]+)@').firstMatch(header);
