...
class _SwitchedCallWidgetState extends State<SwitchedCallWidget> {
  ...
  List<Widget> _buildCallControls() {
    ...
    if (_transferBlindMode) {
      children.add(_buildTransferBlind());
      return children;
    }
    if (_transferAttendedMode) {
      children.add(_buildTransferAttended());
      return children;
    }
          ...
          MenuAnchor(
            ...
            menuChildren: [
              ...
              MenuItemButton(
                ...
                onPressed:
                    context.read<AppCallsModel>().hasConnectedFewCalls()
  ...
  Widget _buildTransferBlind({String action = "Transfer"}) {
    ...
    return Row(
      ...
      children: [
        ...
        OutlinedButton(
          onPressed:
              _transferBlindTextCtrl.text.isEmpty
                  ? null
                  : () {
        ...
        IconButton.filledTonal(
          onPressed: _toggleTransferBlindMode,
          icon: const Icon(Icons.close),
        ),
      ],
    );
  ...
  Widget _buildTransferAttended() {
    ...
    for (var i = 0; i < calls.length; i++) {
      if (srcCallId != calls[i].myCallId) {
        _transferAttendedToCallId = calls[i].myCallId;
        break;
      }
    }
    ...
    return Row(
      ...
      children: [
            ...
            for (var i = 0; i < calls.length; i++)
              if (srcCallId != calls[i].myCallId)
                DropdownMenuItem<int>(
                  value: calls[i].myCallId,
                  child: Text(calls[i].nameAndExt),
                ),
          ],
        ),
        ...
        OutlinedButton(
          onPressed: () {
            _transferAttended(_transferAttendedToCallId);
          },
          child: const Text('Transfer'),
        ),
        IconButton.filledTonal(
          onPressed: _toggleTransferAttendedMode,
          icon: const Icon(Icons.close),
        ),
      ],
    );
